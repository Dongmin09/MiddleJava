...
public class T07EqualsHashcodeTest {
	 ...
	 * 객체가 같은지 여부는 데이터를 추가할때 검사한다.
	    ...
	    2. 두 객체가 같으면 equals() 호출 했을 때 true를 반환해야한다.
	    ...
	    5. hashCode() 는 기본적으로 Heap 메모리에 있는 각 객체에 대한 메모리 주소 매핑정보를
	    기반으로 한 정수값을 반환한다.
	    그러므로, 클래스에서 hashCode()메소드를 override하지 않으면 절대로 두 객체가 같은 것으로
	    간주 될 수 없다.
	 * 
	 * 
	 */
...
//	}
//	
//	// 해쉬코드만 오버라이드 했을 때는 두가지의 값이 나왔으나 equals 문 추가후에는 false 이기 때문에 한개만 나옴.
//	@Override
//	public boolean equals(Object obj) {
//		Person p = (Person) obj;
