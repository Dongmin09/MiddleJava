...
public class T07WhildCartTest {
	...
	public static void displayCartItemInfo3(Cart<? super Meat> cart) {
		System.out.println("= 고기류나 상위류 장바구니 항목 리스트 = ");
		for(Object obj : cart.getList()) {
			System.out.println(obj.toString());			
		}
			System.out.println("=====================");
...
//		displayCartItemInfo2(foodCart);  extends로 드링크 와 그 아래만 호출되게 햇으므로 샹향제한해서 안됨.
//		displayCartItemInfo2(meatCart);
		displayCartItemInfo2(drinkCart);

		displayCartItemInfo3(foodCart);
		displayCartItemInfo3(meatCart);
//		displayCartItemInfo3(drinkCart); super로 하한제한을 해놔서 미트와 그 상위 부분은 호출이 되지만 아래는 안됨.
