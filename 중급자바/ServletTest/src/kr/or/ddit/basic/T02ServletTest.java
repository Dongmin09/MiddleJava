...
public class T02ServletTest extends HttpServlet {
	 ...
	 * 2. 컨테이너는 web.xml에 정의된 url 패턴을 확인하여 어느 서블릿을 통해 
	 * 	    처리할지를 검색한다.(로딩이 안된 경우에는 로딩을 수행한다. 로딩시 init()메서드 호출된다.)
	 * 3. 서블릿 컨테이너는 요청을 처리할 개별 스레드 객체를 생성하여 해당 서블릿 객체의 service()
	 * 	  메서드를 호출한다. (HttpServletRequest 및 HttpServletReponse객체 를 생성하여 파라미터로 넘겨준다)
	 * 4. service() 메서드는 메서드 타입을 체크하여 적절한 메서드를 호출한다.
	 * 	  (doGet, doPost, doPut, doDelete 등)
	 * 5. 요청 처리가 완료되면, HttpServletRequest 및 HttpServletReponse객체는 소멸된다.
	...
	protected void doGet(HttpServletRequest req,
		...
		out.println("서블릿 컨테스트 경로 : " + req.getContextPath());
